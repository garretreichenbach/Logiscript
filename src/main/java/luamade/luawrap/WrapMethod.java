...
public class WrapMethod extends VarArgFunction {
	...
	public WrapMethod(String name_in, Class<?> clazz_in) {
		...
		for(int argc : ms.keySet()) {
			Method m = ms.get(argc);
			Class<?>[] argst = m.getParameterTypes();
			boolean isVrad = argc > 0 && argst[argc - 1] == Varargs.class;
	...
	public Varargs invoke(Varargs vargs) {
		...
		if(methods.containsKey(argc)) return methods.get(argc).invoke(vargs);

		if(varadic != null) return varadic.invoke(vargs);
