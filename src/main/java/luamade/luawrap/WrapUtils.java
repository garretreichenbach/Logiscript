...
public class WrapUtils {
	...
	public static Object unwrapSingle(LuaValue o, Class<?> clazz) {
		...
		if(clazz == String.class) return o.checkstring().tojstring();

		else if(clazz == boolean.class || clazz == Boolean.class) return o.toboolean();

		else if(clazz == double.class || clazz == Double.class) return o.checkdouble();
	...
	public static Object unwrap(LuaValue o, Class<?> clazz) {
		if(clazz.isArray()) return unwrapArray(o, clazz);
		else return unwrapSingle(o, clazz);
	}

	public static Set<String> listMethods(Class<?> clazz) {
		Set<String> out = new HashSet<>();
		for(Method m : clazz.getMethods())
			if(m.isAnnotationPresent(LuaMadeCallable.class)) out.add(m.getName());
		return out;
	}
}
