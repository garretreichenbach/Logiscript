...
public class Shipyard extends LuaMadeUserdata {
	...
	public Boolean isFinished() {
		return isShipyard() && collectionManager.isFinished();
	}

	@LuaMadeCallable
	public Double getCompletion() {
		return isShipyard() ? collectionManager.getCompletionOrderPercent() : 0;
	}

	@LuaMadeCallable
	public Boolean isDocked() {
		return isShipyard() && collectionManager.getCurrentDocked() != null;
	}

	@LuaMadeCallable
	public Boolean isVirtualDocked() {
		return isShipyard() && collectionManager.getCurrentDocked() != null && collectionManager.getCurrentDocked().isVirtualBlueprint();
	}

	@LuaMadeCallable
	public Entity getDocked() {
		if(isDocked()) return Entity.wrap(collectionManager.getCurrentDocked());
		else return null;
	}
	...
	@LuaMadeCallable
	public void undock() {
		if(canUndock()) collectionManager.undockRequestedFromShipyard();
	}
		...
		if(isShipyard()) {
			for(ElementInformation info : ElementKeyMap.getInfoArray()) {
				if(info != null && !info.isDeprecated()) {
					int count = collectionManager.clientGoalTo.get(info.getId());
	...
	public ItemStack[] getCurrent() {
		...
		if(isShipyard()) {
			for(ElementInformation info : ElementKeyMap.getInfoArray()) {
				if(info != null && !info.isDeprecated()) {
					int count = collectionManager.clientGoalFrom.get(info.getId());
	...
	public void sendCommand(String command, Object... args) {
		...
		collectionManager.handleShipyardCommandOnServer(getManagedSegmentController().getFactionId(), type, providedArgs);
	}
